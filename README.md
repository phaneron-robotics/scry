...
&nbsp;
<a href="https://pypi.org/project/scry-connect/"><img src="https://img.shields.io/badge/Install_on_robot-PyPI-3775A9?style=for-the-badge&logo=pypi&logoColor=FFD43B" alt="Install scry-connect from PyPI" height="56"></a>
...
Debugging a ROS 2 robot usually means crouching at a laptop with `rviz`, `rqt`, and a wall of `ros2 topic echo` terminals — fine at your desk, painful when you're walking beside the robot, demoing it, or out in the field. **Scry puts that whole workflow in your pocket:** you ask a question in plain English — *"why isn't `/cmd_vel` publishing?"* — and an on-device AI assistant inspects your robot's topics, nodes, services, parameters, and diagnostics live over your own network, then answers with structured panels, plots, and one-tap actions. No cloud backend, no telemetry — your AI key, your robot, your network.

> **Scry is not a Foxglove replacement.** It's an AI-first *debugging copilot*, not a comprehensive visualization suite.
...
<a href="https://play.google.com/store/apps/details?id=com.phaneronrobotics.scry">Get Scry on Google Play →</a> (or grab the AAB from [Releases](https://github.com/phaneron-robotics/scry-app/releases)).
...
| Provider | Notes |
|---|---|
| [**OpenRouter**](https://openrouter.ai/) | **Recommended.** One key unlocks 300+ models (Claude, GPT, Gemini, Llama, …) and has a free tier. |
...
- **🗣️ Debug by chatting** — *"why isn't `/cmd_vel` publishing?"* — Scry inspects topics, nodes, services, and parameters live and explains what it found.
- **🎙️ Voice & images** — talk hands-free, or attach a screenshot and ask *"what's wrong in this scene?"* Transcription runs on your phone.
- **👆 Act with one tap** — publish a topic, set a parameter, call a service, drive a lifecycle change — every write shows exactly what it'll do and waits for your approval.
- **🔔 Background monitors** — *"alert me if `/odom` drops below 10 Hz"* — Scry watches and pings you the moment a condition trips.
- **📊 Live panels & plots** — sensor readouts, scene snapshots, transform trees, and live plots render right in the chat.
- **🛰️ Fleet view** — monitor and compare multiple robots from one screen.
...
`scry-connect` also ships as multi-arch Docker images on [GHCR](https://github.com/phaneron-robotics/scry-connect/pkgs/container/scry-connect) for **Humble, Jazzy, Kilted, Lyrical, and Rolling**.
